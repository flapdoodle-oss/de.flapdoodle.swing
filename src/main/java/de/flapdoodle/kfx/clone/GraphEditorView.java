...
//    private final SelectionBox mSelectionBox = new SelectionBox();
    ...
    public static String getStyleResource() {
        URL resource = GraphEditorView.class.getResource("defaults.css");
        if (resource==null) throw new IllegalArgumentException("stylesheet not found");
        String externalForm = resource.toExternalForm();
        if (externalForm==null) throw new IllegalArgumentException("could not get external form for "+resource);
        return externalForm;
    }
...
//    }
    ...
    public void add(final DraggableBox box)
    {
        //if (pNodeSkin != null && !(pNodeSkin instanceof VirtualSkin))
        {
            mNodeLayer.getChildren().add(box);
        }
    }
//    /**
